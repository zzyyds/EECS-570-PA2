/******************************
  Program "MSI_Protocal.m" compiled by "Murphi Release 3.1"
...
#define PROTOCOL_NAME "MSI_Protocal"
...
char *mu_1_MessageType::values[] = {"GetS","GetM","Data","FwdGetS","FwdGetM","FwdAck","Inv","InvAck","PutS","PutM","PutAck",NULL };
...
char *mu_1__type_0::values[] = {"H_I","H_S","H_M","H_SM_A","H_MM_A","H_MS_A",NULL };
...
char *mu_1__type_2::values[] = {"P_I","P_IS_D","P_IM_A_D","P_IM_A","P_S","P_SM_A_D","P_SM_A","P_M","P_MI_A","P_SI_A","P_II_A",NULL };
...
const int mu_FwdAck = 12;
const int mu_Inv = 13;
const int mu_InvAck = 14;
const int mu_PutS = 15;
const int mu_PutM = 16;
const int mu_PutAck = 17;
const int mu_H_I = 18;
const int mu_H_S = 19;
const int mu_H_M = 20;
const int mu_H_SM_A = 21;
const int mu_H_MM_A = 22;
const int mu_H_MS_A = 23;
const int mu_P_I = 24;
const int mu_P_IS_D = 25;
const int mu_P_IM_A_D = 26;
const int mu_P_IM_A = 27;
const int mu_P_S = 28;
const int mu_P_SM_A_D = 29;
const int mu_P_SM_A = 30;
const int mu_P_M = 31;
const int mu_P_MI_A = 32;
const int mu_P_SI_A = 33;
const int mu_P_II_A = 34;
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("read from I to S, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
...
bool Condition(unsigned r)
{
  ...
  if (r>=55 && r<=60) return R7.Condition(r-55);
...
void Code(unsigned r)
{
  ...
  if (r>=55 && r<=60) { R7.Code(r-55); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=55 && r<=60) { return R7.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=55 && r<=60) return R7.Name(r-55);
  return NULL;
}
};
...
bool mu__boolexpr18;
bool mu__boolexpr19;
  if (!((mu_HomeNode.mu_state) == (mu_H_S))) mu__boolexpr19 = FALSE ;
  else {
  mu__boolexpr19 = ((mu_Procs[mu_n].mu_state) == (mu_P_S)) ; 
}
...
int mu__invariant_21() // Invariant "values in memory matches value of last write, when shared or invalid"
{
bool mu__quant22; 
mu__quant22 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr23;
  if (!((mu_HomeNode.mu_state) == (mu_H_S))) mu__boolexpr23 = TRUE ;
  else {
...
bool mu__condition_24() // Condition for Rule "values in memory matches value of last write, when shared or invalid"
{
...
int mu__invariant_25() // Invariant "Homenode.state is in I or M implies empty sharer list"
{
bool mu__boolexpr26;
bool mu__boolexpr27;
  if ((mu_HomeNode.mu_state) == (mu_H_I)) mu__boolexpr27 = TRUE ;
  else {
  mu__boolexpr27 = ((mu_HomeNode.mu_state) == (mu_H_M)) ; 
}
...
/*** end multisetcount 6 declaration ***/
  mu__boolexpr26 = ((mu__intexpr28) == (0)) ; 
}
return mu__boolexpr26;
};

bool mu__condition_29() // Condition for Rule "Homenode.state is in I or M implies empty sharer list"
{
...
int mu__invariant_30() // Invariant "modified implies empty sharers list"
{
bool mu__boolexpr31;
  if (!((mu_HomeNode.mu_state) == (mu_H_M))) mu__boolexpr31 = TRUE ;
  else {
...
bool mu__condition_33() // Condition for Rule "modified implies empty sharers list"
{
...
int mu__invariant_38() // Invariant "P.state is in S or M match last write"
{
bool mu__quant39; 
mu__quant39 = TRUE;
{
...
bool mu__boolexpr40;
bool mu__boolexpr41;
  if ((mu_Procs[mu_n].mu_state) == (mu_P_S)) mu__boolexpr41 = TRUE ;
  else {
  mu__boolexpr41 = ((mu_Procs[mu_n].mu_state) == (mu_P_M)) ; 
}
...
bool mu__condition_42() // Condition for Rule "P.state is in S or M match last write"
{
...
int mu__invariant_43() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr44;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr44 = TRUE ;
  else {
  mu__boolexpr44 = ((mu_HomeNode.mu_val) == (mu_LastWrite)) ; 
}
return mu__boolexpr44;
};

bool mu__condition_45() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_46() // Invariant "HomeNode.state is in I or S implies empty owner"
{
bool mu__boolexpr47;
bool mu__boolexpr48;
  if ((mu_HomeNode.mu_state) == (mu_H_I)) mu__boolexpr48 = TRUE ;
  else {
  mu__boolexpr48 = ((mu_HomeNode.mu_state) == (mu_H_S)) ; 
}
  if (!(mu__boolexpr48)) mu__boolexpr47 = TRUE ;
  else {
  mu__boolexpr47 = (mu_HomeNode.mu_owner.isundefined()) ; 
}
return mu__boolexpr47;
};

bool mu__condition_49() // Condition for Rule "HomeNode.state is in I or S implies empty owner"
{
...
{"HomeNode.state is in I or S implies empty owner", &mu__condition_49, NULL, FALSE},
{"value in memory matches value of last write, when invalid", &mu__condition_45, NULL, FALSE},
{"P.state is in S or M match last write", &mu__condition_42, NULL, FALSE},
{"value is undefined while invalid", &mu__condition_37, NULL, FALSE},
{"modified implies empty sharers list", &mu__condition_33, NULL, FALSE},
{"Homenode.state is in I or M implies empty sharer list", &mu__condition_29, NULL, FALSE},
{"values in memory matches value of last write, when shared or invalid", &mu__condition_24, NULL, FALSE},
