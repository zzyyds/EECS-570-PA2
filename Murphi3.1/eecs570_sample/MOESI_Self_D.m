  ...
  ProcState:
    Record
      state: enum { P_I, P_IS_D, P_IM_A_D, P_IM_A,P_IM_D,
                    ...
                    P_OM_A,
                    P_OI_A,
                    P_M, P_MI_A, P_ME_A, P_EI_A, P_SI_A, P_II_A
                  };
      val: Value;
      ack_cnt: Ack_Cnt;
    End;
...
--  put " at home -- "; put HomeNode.state;
  ...
  case H_M:
    switch msg.mtype
    case DowngradeMtoE:
        Send(DowngradeAck, msg.src, HomeType, response_chanel, UNDEFINED, UNDEFINED, msg.src);
        -- undefine HomeNode.owner;
        HomeNode.val :=msg.val;
        HomeNode.state := H_E;
...
case H_E:
    ...
    case DowngradeMtoE:
    --     AddToSharersList(msg.src);
    --     Send(DowngradeAck, msg.src, HomeType, request_chanel, UNDEFINED, UNDEFINED, msg.src);
    --     undefine HomeNode.owner;
        Send(DowngradeAck, msg.src, HomeType, response_chanel, UNDEFINED, UNDEFINED, msg.src);
        HomeNode.val :=msg.val;
        HomeNode.state := H_E;
  ...
  case H_MO_A:
      switch msg.mtype
        ...
        HomeNode.val := msg.val;
        case DowngradeMtoE:
            Send(NACK, msg.src, HomeType, response_chanel, UNDEFINED, UNDEFINED, UNDEFINED);
        case GetS:
          msg_processed := false; -- stall message in InBox
        case GetM:
          msg_processed := false; -- stall message in InBox
...
ruleset n:Proc Do
  ...
  alias pv:Procs[n].val do
    ...
    rule "Self-downgrade (spontaneous M→E)"
    (p.state = P_M) 
  ==>
    Send(DowngradeMtoE, HomeType, n, downgrade_channel, pv, UNDEFINED, UNDEFINED);
    p.state := P_ME_A;
  endrule;
...
invariant "P.state is in S or M or E or O match last write"
  Forall n : Proc Do	
     (Procs[n].state = P_S | Procs[n].state = P_M |Procs[n].state = P_E |Procs[n].state = P_O )
    ->
...
invariant "values in memory matches value of last write, when in H_S, H_I or H_O"
  Forall n : Proc Do	
     (HomeNode.state = H_S | HomeNode.state = H_I | HomeNode.state = H_O) --|  HomeNode.state = H_I | HomeNode.state = H_E
    ->
			HomeNode.val = LastWrite
	end;

invariant "P_S & H_S or P_E & H_E or P_O & H_O match memory"
  Forall n : Proc Do	
     HomeNode.state = H_S & Procs[n].state = P_S | HomeNode.state = H_E & Procs[n].state = P_E | HomeNode.state = H_O & Procs[n].state = P_O
    ->
