  ...
  HomeState:
    Record
      state: enum { H_I, H_S, H_M, 					--stable states
      				 H_SM_A, H_MM_A,H_MS_A }; 								--transient states during recall
      owner: Node;	
...
invariant "P.state is in S or M match last write"
  Forall n : Proc Do	
...
invariant "Homenode.state is in I or M implies empty sharer list"
  (HomeNode.state = H_I | HomeNode.state = H_M)
    ->
      MultiSetCount(i:HomeNode.sharers, true) = 0;
