/******************************
  Program "MESI_Protocal.m" compiled by "Murphi Release 3.1"
...
#define PROTOCOL_NAME "MESI_Protocal"
...
char *mu_1_MessageType::values[] = {"GetS","GetM","Data","E_Data","FwdGetS","FwdGetM","FwdAck","Inv","InvAck","PutS","PutM","PutE","PutAck",NULL };
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (char *name, int os): mu__byte(20, 28, 4, name, os) {};
  mu_1__type_0 (void): mu__byte(20, 28, 4) {};
  mu_1__type_0 (int val): mu__byte(20, 28, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_0::values[] = {"H_I","H_S","H_E","H_M","H_SM_A","H_MM_A","H_MS_A","H_EM_A","H_ES_A",NULL };
...
class mu_1__type_2: public mu__byte
{
 public:
  ...
  mu_1__type_2 (char *name, int os): mu__byte(29, 41, 4, name, os) {};
  mu_1__type_2 (void): mu__byte(29, 41, 4) {};
  mu_1__type_2 (int val): mu__byte(29, 41, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_2::values[] = {"P_I","P_IS_D","P_IM_A_D","P_IM_A","P_S","P_SM_A_D","P_SM_A","P_E","P_M","P_MI_A","P_EI_A","P_SI_A","P_II_A",NULL };
...
const int mu_PutE = 18;
const int mu_PutAck = 19;
const int mu_H_I = 20;
const int mu_H_S = 21;
const int mu_H_E = 22;
const int mu_H_M = 23;
const int mu_H_SM_A = 24;
const int mu_H_MM_A = 25;
const int mu_H_MS_A = 26;
const int mu_H_EM_A = 27;
const int mu_H_ES_A = 28;
const int mu_P_I = 29;
const int mu_P_IS_D = 30;
const int mu_P_IM_A_D = 31;
const int mu_P_IM_A = 32;
const int mu_P_S = 33;
const int mu_P_SM_A_D = 34;
const int mu_P_SM_A = 35;
const int mu_P_E = 36;
const int mu_P_M = 37;
const int mu_P_MI_A = 38;
const int mu_P_EI_A = 39;
const int mu_P_SI_A = 40;
const int mu_P_II_A = 41;
...
break;
case mu_PutM:
if ( !((mu_HomeNode.mu_owner) != (mu_msg.mu_src)) ) Error.Error("Assertion failed: error: src is owner when HomeNode state is Invalid");
mu_Send ( mu_PutAck, mu_msg.mu_src, (int)mu_HomeType, mu_response_chanel, mu_1_Value_undefined_var, mu_1_Ack_Cnt_undefined_var, mu_1_Node_undefined_var );
break;
default:
...
break;
case mu_PutM:
if ( !((mu_msg.mu_src) != (mu_HomeNode.mu_owner)) ) Error.Error("Assertion failed: Writeback from owner rather than nonOwner");
if ( mu_IsSharer( mu_msg.mu_src ) )
{
if ( (mu_cnt) == (1) )
{
...
mu_HomeNode.mu_state = mu_H_MS_A;
break;
case mu_GetM:
...
mu_AddToSharersList ( mu_msg.mu_src );
mu_AddToSharersList ( mu_HomeNode.mu_owner );
mu_HomeNode.mu_state = mu_H_ES_A;
break;
case mu_GetM:
...
bool Condition(unsigned r)
{
  ...
  if (r>=61 && r<=66) return R9.Condition(r-61);
...
void Code(unsigned r)
{
  ...
  if (r>=61 && r<=66) { R9.Code(r-61); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=61 && r<=66) { return R9.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=61 && r<=66) return R9.Name(r-61);
  return NULL;
}
};
...
/********************
  Invariant records
 ********************/
int mu__invariant_16() // Invariant "values in shared state or E match memory"
{
bool mu__quant17; 
mu__quant17 = TRUE;
{
...
bool mu__boolexpr19;
bool mu__boolexpr20;
  if (!((mu_HomeNode.mu_state) == (mu_H_S))) mu__boolexpr20 = FALSE ;
  else {
  mu__boolexpr20 = ((mu_Procs[mu_n].mu_state) == (mu_P_S)) ; 
}
  if (mu__boolexpr20) mu__boolexpr19 = TRUE ;
  else {
bool mu__boolexpr21;
  if (!((mu_HomeNode.mu_state) == (mu_H_E))) mu__boolexpr21 = FALSE ;
  else {
  mu__boolexpr21 = ((mu_Procs[mu_n].mu_state) == (mu_P_E)) ; 
}
  mu__boolexpr19 = (mu__boolexpr21) ; 
}
...
bool mu__condition_22() // Condition for Rule "values in shared state or E match memory"
{
...
int mu__invariant_23() // Invariant "values in memory matches value of last write, when shared or invalid"
{
bool mu__quant24; 
mu__quant24 = TRUE;
{
...
bool mu__boolexpr25;
bool mu__boolexpr26;
  if ((mu_HomeNode.mu_state) == (mu_H_S)) mu__boolexpr26 = TRUE ;
  else {
  mu__boolexpr26 = ((mu_HomeNode.mu_state) == (mu_H_I)) ; 
}
  if (!(mu__boolexpr26)) mu__boolexpr25 = TRUE ;
  else {
  mu__boolexpr25 = ((mu_HomeNode.mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr25) )
...
bool mu__condition_27() // Condition for Rule "values in memory matches value of last write, when shared or invalid"
{
...
int mu__invariant_28() // Invariant "Homenode.state is in I or M or Eimplies empty sharer list"
{
bool mu__boolexpr29;
bool mu__boolexpr30;
bool mu__boolexpr31;
  if ((mu_HomeNode.mu_state) == (mu_H_I)) mu__boolexpr31 = TRUE ;
  else {
  mu__boolexpr31 = ((mu_HomeNode.mu_state) == (mu_H_M)) ; 
}
  if (mu__boolexpr31) mu__boolexpr30 = TRUE ;
  else {
  mu__boolexpr30 = ((mu_HomeNode.mu_state) == (mu_H_E)) ; 
}
...
bool mu__condition_33() // Condition for Rule "Homenode.state is in I or M or Eimplies empty sharer list"
{
...
int mu__invariant_34() // Invariant "value is undefined while invalid"
{
bool mu__quant35; 
mu__quant35 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr36;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_I))) mu__boolexpr36 = TRUE ;
  else {
  mu__boolexpr36 = (mu_Procs[mu_n].mu_val.isundefined()) ; 
}
if ( !(mu__boolexpr36) )
...
bool mu__condition_37() // Condition for Rule "value is undefined while invalid"
{
...
int mu__invariant_38() // Invariant "P.state is in S or M or E match last write"
{
bool mu__quant39; 
mu__quant39 = TRUE;
{
...
bool mu__boolexpr41;
bool mu__boolexpr42;
  if ((mu_Procs[mu_n].mu_state) == (mu_P_S)) mu__boolexpr42 = TRUE ;
  else {
  mu__boolexpr42 = ((mu_Procs[mu_n].mu_state) == (mu_P_M)) ; 
}
  if (mu__boolexpr42) mu__boolexpr41 = TRUE ;
  else {
  mu__boolexpr41 = ((mu_Procs[mu_n].mu_state) == (mu_P_E)) ; 
}
...
bool mu__condition_43() // Condition for Rule "P.state is in S or M or E match last write"
{
...
int mu__invariant_44() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr45;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr45 = TRUE ;
  else {
...
bool mu__condition_46() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_47() // Invariant "HomeNode.state is in I or S implies empty owner"
{
bool mu__boolexpr48;
bool mu__boolexpr49;
  if ((mu_HomeNode.mu_state) == (mu_H_I)) mu__boolexpr49 = TRUE ;
  else {
  mu__boolexpr49 = ((mu_HomeNode.mu_state) == (mu_H_S)) ; 
}
...
bool mu__condition_50() // Condition for Rule "HomeNode.state is in I or S implies empty owner"
{
...
{"HomeNode.state is in I or S implies empty owner", &mu__condition_50, NULL, FALSE},
{"value in memory matches value of last write, when invalid", &mu__condition_46, NULL, FALSE},
{"P.state is in S or M or E match last write", &mu__condition_43, NULL, FALSE},
{"value is undefined while invalid", &mu__condition_37, NULL, FALSE},
{"Homenode.state is in I or M or Eimplies empty sharer list", &mu__condition_33, NULL, FALSE},
{"values in memory matches value of last write, when shared or invalid", &mu__condition_27, NULL, FALSE},
{"values in shared state or E match memory", &mu__condition_22, NULL, FALSE},
};
